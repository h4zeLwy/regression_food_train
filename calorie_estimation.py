...
if __name__=='__main__':
    ...
    y = []
    food = []
    label = .(os.path.join(args.path, 'csv', args.option+'.csv'))
    ...
    if args.option=='train':
        ...
        model.save("model_estimation.h5")
    elif args.option=='evo':
                model = load_model('modelestimation.h5',compile=False)
                model.get_weights()
                ...
                """model = load_model('modelestimation.h5',compile=False)
                #model.layers[dense_1].output.get_shape()
                inp = model.input
